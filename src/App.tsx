...
function App() {
  ...
  const handleKeyPress = (e: React.KeyboardEvent<HTMLInputElement>): void => {
    if (e.key === "Enter") {
      ...
      const isInTags = tags.find(
        (e) => e.name.toLowerCase() === inputValue.toLowerCase()
      );
      if (color && !isInTags) {
        setTags((previous) => [...previous, color]);
        setInputValue("");
      }
    }
  };
  const deleteTag = (tagName: string): void => {
    setTags(tags.filter((e) => e.name.toLowerCase() !== tagName.toLowerCase()));
  };
  return (
    <div className="App">
        ...
        {tags.map((tag) => {
          return (
            <div
              className="tag"
              key={tag.hex}
              style={{ backgroundColor: `#${tag.hex}`, color: "#ffffff" }}
            >
              <div aria-label="tag name">{tag.name}</div>
              <div
                className="deleteTag"
